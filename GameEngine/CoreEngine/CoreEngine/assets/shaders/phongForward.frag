...
void main()
{
	...
	mat4 rotation = cameraTransform * objectTransform;
	...
	if (textureEnabled)
	{
		...
		if (blendTexture)
			albedo = blendColor(pixelColor * textureColor, albedo);
		else
			albedo *= pixelColor* textureColor;
	}
	...
	mat3 accumulation = mat3(
		albedo.rgb * globalDiffuse * materialDiffuse.rgb * max(lightDot, 0),
		globalSpecular * specular * pow(max(dot(reflected, look), 0), shininess),
		globalAmbient * materialAmbient.rgb * albedo.rgb
	);
	...
	vec3 alphaChannels = vec3(
		albedo.a + (1 - albedo.a) * min(getValue(accumulation[0]) * albedo.a, 1),
		albedo.a + (1 - albedo.a) * min(getValue(accumulation[1]) * albedo.a, 1),
		albedo.a + (1 - albedo.a) * min(getValue(accumulation[2]) * albedo.a, 1)
	);
	...
	vec3 localIntensity = glowColor.rgb + globalEmissive + materialEmissive * albedo.rgb + intensity;
	
	outputColor = vec4(localIntensity, maxAlpha);
	glowOutputColor = vec4(materialEmissive * color.rgb, 1);
