...
void main()
{
	...
	if (useOffsetMap)
	{
		...
		normal = (rotation * vec4(normalize(cross(vec3(d, offsetData.y, 0), vec3(0, offsetData.z, d))), 0)).xyz;
	}
	...
	deferredDiffuse = vec4(albedo.xyz * materialDiffuse, float(shininess) / 255);
	deferredSpecular = vec4(specular, 1);
	deferredAmbient = vec4(albedo.xyz * materialAmbient, 1);
	deferredEmissive = vec4(materialEmissive, 1);
	deferredGlow = vec4(materialEmissive * color.xyz, 1);
